...
public static class RateLimitingMiddlewareValidation
{
    ...
    public static IReadOnlyList<string> Validate(this TokenBucket bucket)
    {
        ...
        // These are internal fields, so we validate the constructor parameters
        // which are validated during construction
    ...
    /// <returns>True if valid; otherwise, false.</returns>
    public static bool IsValid(this RateLimitingOptions options)
    {
    ...
    /// <returns>True if valid; otherwise, false.</returns>
    public static bool IsValid(this TokenBucket bucket)
    {
        return bucket.Validate().Count == 0;
    }
    ...
    public static void EnsureValid(this RateLimitingOptions options)
    {
        ...
        {
            throw new ArgumentException(
                $"RateLimitingOptions is not valid. Problems:\n{string.Join("\n", problems)}");
        }
    }
    ...
    public static void EnsureValid(this TokenBucket bucket)
    {
        ...
        var problems = bucket.Validate();
        if (problems.Count > 0)
        {
            throw new ArgumentException(
                $"TokenBucket is not valid. Problems:\n{string.Join("\n", problems)}");
        }
    }
}
